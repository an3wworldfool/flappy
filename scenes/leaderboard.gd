...
func fill_leaderboard(records_arr: Array) -> void:
	...
	for i in range(len(records_arr)):
		...
		row.set_text(1, sorted_arr[i]["username"]["S"])
		row.set_text(2, str(sorted_arr[i]["high_score"]["N"]))
		row.set_text(3, sorted_arr[i]["score_date"]["S"])
	if(new_record):
...
func ranking_sorting(my_arr: Array) -> Array:
	for i in range(my_arr.size()):
		if(i+1 < my_arr.size()):
			var a = my_arr[i]
			var b = my_arr[i+1]
			if(a["high_score"]["N"] == b["high_score"]["N"] and a["score_date"]["S"].naturalnocasecmp_to(b["score_date"]["S"])>0):
				my_arr[i+1] = a
				my_arr[i] = b
	return my_arr	
